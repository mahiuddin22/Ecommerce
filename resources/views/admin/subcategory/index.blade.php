                ...
                <table class="table table-striped table-bordered bootstrap-datatable datatable">
                    ...
                    @foreach($subcategories as $key=>$subcategory)
                        <tr style="box-shadow: 2px 2px 2px #CCCCCC;">
                            ...
                            <td style="border-bottom: 1px solid #c4c4c4;" class="center">
                                @if($subcategory->status == 1)
                                    <a class="btn btn-danger" href="{{route('subcatstatus',$subcategory->id)}}">
                                        <i class="halflings-icon white eye-close"></i>
                                    </a>
                                @else
                                    <a class="btn btn-success" href="{{route('subcatstatus',$subcategory->id)}}">
                                        <i class="halflings-icon white eye-open"></i>
                                    </a>
                                @endif
                                <a class="btn btn-info" href="{{route('sub.edit', $subcategory->id)}}">
                                    <i class="halflings-icon white edit"></i>
                                </a>
                                <form action="{{route('sub.delid', $subcategory->id)}}" method="post" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
